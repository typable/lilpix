...
fn main() -> error::Result<()> {
    ...
    let mut index = 0;
    let size = 32;
    let mut layers: Vec<Vec<Option<(u8, u8, u8)>>> = vec![
        vec![None; size * size],
        vec![None; size * size],
        vec![None; size * size],
        vec![None; size * size],
    ...
    for (i, color) in colors.iter().enumerate() {
        ...
        term.write(format!("\x1b[48;2;{r};{g};{b}m\x1b[38;2;0;0;0m{symbol}\x1b[0m").as_bytes())?;
    }
    // for y in 0..10 {
    //     for x in 0..10 {
        ...
        if let Retrieved::Event(Some(event)) = term.get(Value::Event(None))? {
            match event {
                Event::Mouse(mouse) => match mouse {
                    MouseEvent::Up(MouseButton::Left, col, row, _)
                    | MouseEvent::Drag(MouseButton::Left, col, row, _) => {
                        if col >= padding * 2
                            ...
                            && row < padding + size as u16
                        {
                            match mode {
                                Mode::Insert => {
                                    ...
                                    term.flush()?;
                                    layers[active][y as usize * size + x as usize] =
                                        Some((r, g, b));
                                }
                                Mode::Erase => {
                                    let y = row - padding;
                                    ...
                                    term.flush()?;
                                    layers[active][y as usize * size + x as usize] = None;
                                }
                            }
                        }
                ...
                Event::Key(key) => match key.code {
                    KeyCode::Tab => {
                        match mode {
                            Mode::Insert => mode = Mode::Erase,
                            Mode::Erase => mode = Mode::Insert,
                        }
                        let mode_symbol = match mode {
                            Mode::Insert => "INS",
                            Mode::Erase => "ERS",
                        ...
                        term.write(
                            format!(
                                "{: <offset$}[{size}x{size}]",
                                format!("[{mode_symbol}]"),
                                offset = (size as u16 * 2
                                    - ((size.to_string().len() * 2 + 3) as u16))
                                    as usize
                            )
                            .as_bytes(),
                        )?;
                    ...
                    KeyCode::Right => {
                        if active < 4 {
                            ...
                            for y in 0..size {
                                for x in 0..size {
                                    if layers[active][y * size + x].is_none() {
                                        term.act(Action::MoveCursorTo(
                                            padding * 2 + x as u16 * 2,
                                            padding + y as u16,
                                        ))?;
